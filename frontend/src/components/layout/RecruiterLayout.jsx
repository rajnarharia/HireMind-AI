...
import { 
  LayoutDashboard, Users, UserPlus, LogOut, Menu, X, ChevronLeft, Sun, Moon,
  Search, User, Settings, Filter, FileText, BrainCircuit, Code2, Sparkles, Briefcase
} from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] flex overflow-hidden transition-colors duration-300">
      ...
      <motion.aside 
        ...
        className="hidden md:flex flex-col bg-white dark:bg-[var(--surface)] border-r border-gray-200 dark:border-white/5 z-30 transition-all duration-300 relative"
      >
        ...
        <button 
          ...
          className="absolute -right-3 top-6 bg-white dark:bg-[var(--surface)] border border-gray-200 dark:border-white/5 rounded-full p-1 shadow-sm hover:scale-110 transition-transform z-40 text-gray-600 dark:text-gray-400"
        >
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-lg font-medium transition-all group relative overflow-hidden ${
                  isActive 
                    ...
                    : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/5 hover:text-gray-900 dark:text-white'
                }`
              }
            >
              {({ isActive }) => (
                <>
                  <item.icon className={`w-5 h-5 shrink-0 transition-colors ${isActive ? 'text-primary' : 'text-gray-400'}`} />
                  <AnimatePresence>
                    {sidebarOpen && (
                      <motion.span 
      ...
      {/* Mobile Header & Menu */}
      <div className="md:hidden fixed top-0 w-full z-40 bg-white dark:bg-[var(--surface)]/80 backdrop-blur-xl border-b border-gray-200 dark:border-white/5">
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -20 }}
            className="md:hidden fixed inset-0 z-30 pt-16 bg-[var(--background)]"
          >
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-4 py-3 rounded-xl font-medium ${
                      isActive ? 'bg-gray-200/50 dark:bg-white/10 text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400'
                    }`
                  }
                >
      ...
      {/* Main Content Area */}
      <main className="flex-1 flex flex-col min-w-0 bg-gray-50/50 dark:bg-transparent overflow-hidden md:border-l md:border-gray-200 dark:border-white/5 md:mt-2 shadow-2xl relative z-10">
        
        {/* Top Navbar */}
        <header className="h-16 flex items-center justify-between px-8 bg-white dark:bg-[var(--surface)] sticky top-0 z-20 border-b border-gray-200 dark:border-white/5">
          
          <div className="flex items-center gap-4 bg-[var(--background)] rounded-full px-4 py-1.5 border border-gray-200 dark:border-white/5 cursor-pointer hover:border-primary/50 transition-colors" onClick={() => window.dispatchEvent(new CustomEvent('open-command-palette'))}>
            <Search className="w-4 h-4 text-gray-400" />
            <input 
              type="text" 
              placeholder="Search Candidates... (Ctrl+K)" 
              readOnly
              className="bg-transparent border-none outline-none text-sm w-48 text-gray-900 dark:text-white placeholder-text-muted cursor-pointer pointer-events-none"
            />
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative group">
              ...
              <div id="recruiter-profile-dropdown" className="hidden absolute right-0 mt-2 w-48 bg-white dark:bg-[var(--surface)] rounded-xl shadow-xl py-2 border border-gray-200 dark:border-white/10 z-50">
